...
compare.interest.hist <- function(train.data, feature) {
  p <- ggplot() + 
    geom_histogram(aes(x=unlist(train.data[[feature]]), 
                       fill=unlist(train.data[["interest_level"]])
                       ),
                   alpha=0.5,
                   bins = max(unlist(train.data[[feature]])),
                   binwidth = 0.5
                   ) +
    xlab(feature) + 
    ggtitle(paste(c("Histogram of", feature), collapse = " ")) +
...
compare.interest.hist(train.data, "price")

ggplot(train.data) + geom_point(aes(x=price, y=bathrooms, color=interest_level)) + scale_x_log10()
...
# testing values 3, 10, ~200, 1000

features$common <- ifelse(features$frequency > 2400, "Common", "Uncommon")
...
listing.features.commoncounts <- inner_join(listing.features.commoncounts, 
                                            select(train.data, listing_id, interest_level), 
                                            by="listing_id")
...
listing.features.commoncounts2 <- 
  inner_join(listing.features2, select(train.data, listing_id, interest_level), by="listing_id") %>%
  group_by(f.text) %>% 
...
listing.features.commoncounts2 %>% filter(grepl("manager", f.text)) %>% summarise(Freq = sum(frequency),
                                                                               High = sum(High)*100, 
                                                                               Medium = sum(Medium)*100, 
                                                                               Low = sum(Low)*100)

## It seems as though features are always used more in Low interest level listings
